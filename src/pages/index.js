  ...
  render() {
    return (
      <StaticQuery
        query={graphql`
          query Candidates {
            allCandidatesYaml(filter: {status: {eq: "active"}}, sort: {fields: name}) {
              nodes {
                name
                short_name
                character_id
        ...
        render={data => (
          <Layout timeout={this.state.timeout} loading={this.state.loading}>
            <Header timeout={this.state.timeout}><h2 className="align-center">YC122 Federation Elections</h2></Header>
            <div className="main" id="main">
              <Carousel
                ...
                >
                {data.allCandidatesYaml.nodes.map((node, index) => (
